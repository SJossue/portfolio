...
// Faithful TypeScript port of the React Bits "Hyperspeed" demo.
// The internal shader/scene plumbing keeps loose typing to match the upstream
// reference; only the React-facing API is strictly typed.
...
export type HyperspeedDistortion =
  | 'turbulentDistortion'
...
export default function Hyperspeed({ effectOptions, className }: HyperspeedProps) {
  ...
  useEffect(() => {
    ...
    const distortions: Record<HyperspeedDistortion, any> = {
      mountainDistortion: {
    ...
    const distortionKey = merged.distortion as HyperspeedDistortion;
    merged.distortion = distortions[distortionKey];
  ...
  return <div ref={containerRef} className={`hyperspeed-canvas ${className ?? ''}`} />;
}
