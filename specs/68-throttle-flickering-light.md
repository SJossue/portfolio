# Perf: Throttle FlickeringLight to every 3rd frame
...
`FlickeringLight` runs `Math.random()` and updates material + light properties every single frame at 60fps. A fluorescent light flicker looks identical at 20fps. Throttle the useFrame callback to only execute every 3rd frame.
...
Replace the entire content of `src/components/features/scene/FlickeringLight.tsx` with:
...
- `FlickeringLight` only updates every 3rd frame via `frameCount.current % 3 !== 0`
- Visual flicker effect is preserved
