# Perf: Optimize NeonOpenSign useFrame — no traverse
...
`NeonOpenSign` calls `.traverse()` on ~29 meshes every frame to apply a strobe effect. Replace with a ref-based material array approach identical to the NeonNameSign optimization.
...
Replace the entire content of `src/components/features/scene/NeonOpenSign.tsx` with:
...
- `NeonOpenSign` no longer uses `.traverse()`
- Materials are registered via callback refs
- `useFrame` iterates a flat array
